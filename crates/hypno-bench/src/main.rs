//! Hypno Performance Benchmark Suite
//!
//! Measures:
...
use hypno_inference::kernels::{self, cpu_features, matmul_f32, matmul_q4_0, matmul_f16};
use hypno_quantize;
use half::f16;
...
fn main() {
    ...
    let json_path = "/tmp/hypno_bench_results.json";
...
fn bench_rms_norm(dim: usize) -> BenchResult {
    ...
    let t0 = Instant::now();
    for _ in 0..runs {
        hypno_inference::kernels::rms_norm_fused(&mut x, &w, 1e-5);
    }
    let elapsed = t0.elapsed().as_secs_f64() / runs as f64;
...
fn bench_softmax(dim: usize) -> BenchResult {
    ...
    let t0 = Instant::now();
    for _ in 0..runs {
        hypno_inference::kernels::softmax_fast(&mut x);
    }
    let elapsed = t0.elapsed().as_secs_f64() / runs as f64;
...
fn bench_quantize_speed(n: usize) -> BenchResult {
    ...
    let t0 = Instant::now();
    for _ in 0..runs {
        let _ = hypno_quantize::quantize_f32_to_q4_0(&data);
    }
    let elapsed = t0.elapsed().as_secs_f64() / runs as f64;
