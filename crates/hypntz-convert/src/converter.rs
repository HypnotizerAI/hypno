        ...
        for t in &tensors {
            if let Ok(view) = sf.tensor(&t.name) {
                ...
                match edt {
                    ...
                    DType::Q4_0 => {
                        buffer = hypntz_quantize::quantize_f32_to_q4_0(&src_data);
                    }
                    DType::Q8_0 => {
                        buffer = hypntz_quantize::quantize_f32_to_q8_0(&src_data);
                    }
                }
...
pub fn validate(path: &Path) -> anyhow::Result<()> {
    let model = hypntz_loader::HypnoModel::open(path)
...
mod tests {
    ...
    fn test_convert_and_validate() {
        ...
        // Check with loader
        let model = hypntz_loader::HypnoModel::open(&out_path).unwrap();
